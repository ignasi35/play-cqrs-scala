...
final class Tagger[Event](tagGroups: List[TagGroup[Event]] = Nil) {
    ...
    tagGroups
      ...
      .map { tagGroup =>
        if (tagGroup.numOfShards > 1) {
          val shardedTag =
            for { shardId <- 0 until tagGroup.numOfShards } yield
              tagGroup.shardTag(shardId)
          shardedTag.toSet
  ...
  def addTagGroup(tag: String,
                  numOfShards: Int,
                  predicate: Event => Boolean = _ => true): Tagger[Event] =
...
@ApiMayChange
final class TagGroup[Event](val originalTag: String,
                            val numOfShards: Int,
  ...
  final def tagFunction(persistenceId: String): Event => Option[String] =
    evt => {
      if (predicate(evt))
        Some(shardTag(Math.abs(persistenceId.hashCode % numOfShards)))
      else None
    }
}
